        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl shadow-xs divide-y divide-slate-100 overflow-hidden">
          {filteredCustomers.length === 0 ? (
            ...
            filteredCustomers.map((customer) => {
              ...
              const customerSalesList = sales.filter(
                (s) =>
                  s.customer_id === customer.id ||
                  (customer.phone && customer.phone !== 'N/A' && s.customer_phone === customer.phone)
              );
              const lastSale = customerSalesList[0];
              ...
              return (
                ...
                >
                  ...
                  >
                    <div className="w-11 h-11 rounded-2xl bg-slate-900 text-white font-black text-base flex items-center justify-center border border-orange-500 flex-shrink-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center space-x-2">
                        ...
                        {customerSalesList.length > 0 && (
                          <span className="text-[10px] font-bold bg-slate-100 text-slate-700 px-2 py-0.5 rounded-full">
                            {customerSalesList.length} Bills
                          </span>
                        )}
                      </div>
                      ...
                      {lastItem && (
                        <p className="text-[10px] text-orange-700 font-semibold flex items-center gap-1 mt-0.5 truncate">
                          <Footprints className="w-3 h-3 flex-shrink-0" />
                          <span>Last: {lastItem.item_name} {lastItem.size ? `(Size ${lastItem.size})` : ''}</span>
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end space-x-3 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100">
                    <div className="text-left sm:text-right">
                      {bal > 0 ? (
                        <>
                          <p className="text-xs sm:text-sm font-black text-amber-800 font-mono">
                            ₹{bal.toLocaleString('en-IN')}
                          </p>
                          <span className="text-[10px] font-bold text-amber-700 block">Due Receivable</span>
                        </>
                      ) : (
                        <>
                          ...
                          <span className="text-[10px] font-bold text-emerald-600 block">Settled</span>
                        </>
                      )}
                    </div>
                    ...
                    <button
                      ...
                      className="px-3 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-2xs transition-all cursor-pointer"
                      title="Start Sale for this Customer in POS"
                    >
                    ...
                    <button
                      ...
                      title="View Customer Khata & Purchases"
                    >
