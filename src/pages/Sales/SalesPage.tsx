...
export const SalesPage: React.FC = () => {
  ...
  const { sales, customers, addCustomer, deleteCustomer, activeShop, userProfile, activeRole } = useShop();
  ...
  return (
    <PermissionGuard requiredPermission="sales:view">
      <div className="space-y-6 max-w-7xl mx-auto pb-10 font-sans">
          ...
          <div className="flex items-center gap-2.5 w-full sm:w-auto flex-1">
            <div className="relative flex-1 sm:max-w-md">
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && activeTab === 'sales' && filteredSales.length > 0) {
                    setSelectedReceipt(filteredSales[0]);
                  }
                }}
                placeholder={activeTab === 'sales' ? 'Search invoice # (e.g. ZAIN-1025), customer, phone... (Press Enter to open)' : 'Search customer by name or WhatsApp phone...'}
            ...
            {activeTab === 'sales' && searchQuery.trim().length > 0 && filteredSales.length === 1 && (
              <button
                type="button"
                onClick={() => setSelectedReceipt(filteredSales[0])}
                className="hidden sm:flex items-center gap-1.5 px-3 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-xl text-xs font-bold transition-all shadow-xs cursor-pointer animate-in fade-in"
              >
                <Eye className="w-3.5 h-3.5" />
                <span>Open #{filteredSales[0].receipt_number}</span>
              </button>
            )}
        ...
        {activeTab === 'sales' && (
          isAdmin ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-2xs space-y-0.5">
                ...
                <p className="text-[11px] text-slate-500 font-medium">
                  {filteredSales.length} order{filteredSales.length === 1 ? '' : 's'} ({formatDateLabel(dateFilter)})
                </p>
              </div>
              ...
              <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-2xs space-y-0.5">
                ...
                <p className="text-[11px] text-slate-500 font-medium">
                  Avg. Ticket ₹{filteredSales.length > 0 ? Math.round(totalSalesAmount / filteredSales.length).toLocaleString('en-IN') : 0} / bill
                </p>
              </div>
            </div>
          ) : (
            <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-2xs flex items-center justify-between">
              <div>
                <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider block">Counter Orders</span>
                <p className="text-2xl font-black text-orange-600 font-mono">{filteredSales.length} Orders</p>
              </div>
              ...
              <table className="w-full text-left text-xs font-medium border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 text-slate-400 font-extrabold uppercase text-[10px] bg-slate-50/50">
                    <th className="py-3.5 px-5">Invoice / Receipt #</th>
                    <th className="py-3.5 px-5">Date & Time</th>
                    <th className="py-3.5 px-5">Customer Profile</th>
                    <th className="py-3.5 px-5">Items & Sizes</th>
                    ...
                    >
                      <td className="py-4 px-5">
                        <button
                          ...
                          className="font-mono font-black text-slate-900 group-hover:text-[#ff6600] flex items-center gap-1.5 transition-colors cursor-pointer text-xs"
                        >
                        ...
                        <span className="text-[10px] text-slate-400 font-bold block mt-0.5">
                          {sale.items?.length || 1} Item(s) • Click for Breakdown
                        </span>
                      </td>
                      ...
                      <td className="py-4 px-5">
                        <div className="flex flex-wrap gap-1 max-w-xs">
                          {(sale.items && sale.items.length > 0
                            ? sale.items
                            : [{ item_name: 'Footwear Sale', size: 'Std' }]
                          ).slice(0, 2).map((it, idx) => (
                            <span
                              key={idx}
                              className="inline-flex items-center gap-1 px-2 py-0.5 bg-slate-100 text-slate-800 rounded-md text-[10px] font-bold border border-slate-200"
                            >
                              <span>{it.item_name.split(' (')[0]}</span>
                              {it.size && (
                                <span className="text-orange-600 font-black">[{it.size}]</span>
                              )}
                            </span>
                          ))}
                      ...
                      <td className="py-4 px-5 text-[11px]">
                        <div className="space-y-0.5">
                          ...
                          {(sale.due_amount || 0) > 0 && (
                            <span className="inline-block bg-rose-50 text-rose-900 border border-rose-200 px-2 py-0.5 rounded font-extrabold ml-1">
                              Due: ₹{(sale.due_amount || 0).toLocaleString('en-IN')}
                            </span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-5 text-right space-x-2 whitespace-nowrap">
                        <button
                          ...
                          className="px-2.5 py-1.5 bg-slate-900 hover:bg-black text-white font-extrabold rounded-xl text-[11px] transition-colors inline-flex items-center gap-1 shadow-2xs cursor-pointer"
                        >
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleShareReceiptWhatsApp(sale);
                          }}
                          className="px-2.5 py-1.5 bg-orange-50 hover:bg-orange-100 text-orange-900 border border-orange-300 font-extrabold rounded-xl text-[11px] transition-colors inline-flex items-center gap-1 cursor-pointer"
                        >
            ...
            <div className="block md:hidden p-4 space-y-3">
              {filteredSales.map((sale) => (
                <div
                  key={sale.id}
                  onClick={() => setSelectedReceipt(sale)}
                  className="p-4 bg-slate-50 hover:bg-orange-50/40 active:scale-[0.99] rounded-2xl border border-slate-200 space-y-2 cursor-pointer transition-all"
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] font-extrabold font-mono text-orange-600 bg-orange-100/70 px-2 py-0.5 rounded-md">
                          #{sale.receipt_number}
                        </span>
                      ...
                      <h4 className="font-extrabold text-slate-900 text-sm mt-1">{sale.customer_name || 'Walk-in Customer'}</h4>
                  ...
                  {/* Footwear items preview chips */}
                  <div className="flex flex-wrap gap-1 pt-1">
                    {(sale.items || [{ item_name: 'Footwear Sale', size: 'Std' }]).slice(0, 3).map((it, idx) => (
                      <span key={idx} className="text-[10px] font-bold bg-white px-2 py-0.5 rounded border border-slate-200 text-slate-700">
                        {it.item_name.split(' (')[0]} {it.size ? `[UK ${it.size}]` : ''}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex justify-between items-center text-xs pt-2 border-t border-slate-200">
                    ...
                    <div className="flex gap-1.5" onClick={(e) => e.stopPropagation()}>
                      <button
                        onClick={() => setSelectedReceipt(sale)}
                        className="px-2.5 py-1 bg-slate-900 text-white font-extrabold rounded-lg text-[11px] flex items-center gap-1 cursor-pointer"
                      >
                        <Eye className="w-3 h-3 text-orange-400" /> View
                      </button>
                      <button
                        onClick={() => handleShareReceiptWhatsApp(sale)}
                        className="px-2.5 py-1 bg-[#ff6600] text-white font-extrabold rounded-lg text-[11px] flex items-center gap-1 cursor-pointer"
                      >
        ...
        {/* INVOICE & ORDER DETAILS BREAKDOWN MODAL */}
        <InvoiceDetailModal
